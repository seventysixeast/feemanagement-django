 ...
 */
  ...
  /* if (feesForMonthsField && feesPeriodMonthField) {
    function updateFeesPeriodMonths() {
      // Get selected months from feesForMonthsField
      const selectedFeesForMonths = Array.from(feesForMonthsField.selectedOptions)
        .map(option => option.value) // Get the selected values (e.g., "4,5,6")
        .flatMap(months => months.split(',')); // Split values by commas to get individual months
      ...
      // Add each individual month as a new option
      selectedFeesForMonths.forEach(month => {
        let option = document.createElement('option');
        option.value = month;  // This should be '1', '2', etc.
        option.textContent = month;
    ...
    // Initial population of fees_period_month on page load
    updateFeesPeriodMonths();
  } else {
    console.error('Element(s) not found for fees_for_months or fees_period_month');
  } */
  ...
  /*  function updateFeesPeriodMonths() {
     const selectedFeesForMonths = Array.from(feesForMonthsField.selectedOptions).map(option => option.value);
     const selectedFeesPeriodMonths = Array.from(feesPeriodMonthField.selectedOptions).map(option => option.value);
 
     // Check if both fees_for_months and fees_period_month have selected values
     if (selectedFeesForMonths.length > 0 && selectedFeesPeriodMonths.length > 0) {
       // Clear the current options in feesPeriodMonthField
       feesPeriodMonthField.innerHTML = '';
 
       // Add only the months that match in both fields
       selectedFeesForMonths.forEach(month => {
         if (selectedFeesPeriodMonths.includes(month)) {
           let option = document.createElement('option');
           option.value = month;
           option.textContent = month;
           feesPeriodMonthField.appendChild(option);
       ...
       // If no matching months, reset to default selection or clear
       if (feesPeriodMonthField.options.length === 0) {
         let defaultOption = document.createElement('option');
         defaultOption.value = '';
         defaultOption.textContent = 'No matching months';
         feesPeriodMonthField.appendChild(defaultOption);
       }
     }
   }
   ...
   feesPeriodMonthField.addEventListener('change', function () {
     updateFeesPeriodMonths();
   });
 
   // Initial population of the fields on page load
   updateFeesPeriodMonths(); */
